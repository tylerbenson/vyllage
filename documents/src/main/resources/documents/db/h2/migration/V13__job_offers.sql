...
create table if not exists DOCUMENTS.job_responsibility(
	job_responsibility_id bigint generated by default as identity(start with 0) primary key,
	job_offer_id bigint not null,
	description varchar_ignorecase(4096),
  	date_created timestamp not null,
  	last_modified timestamp not null);
  	
insert into DOCUMENTS.job_offers(job_offer_id, user_id, organization_id, job_type, job_experience, salary, location, 
company, role, description, 
requires_relocation, remote, date_created, last_modified)
...
insert into DOCUMENTS.job_responsibility(job_offer_id, description, date_created, last_modified) 
values(0, 'Troubleshoot and problem solve issues for Google''s corporate resources, tools, operating systems and applications both locally and remotely.',
CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP());

insert into DOCUMENTS.job_responsibility(job_offer_id, description, date_created, last_modified) 
values(0, 'Work with service owners and customer teams to plan and implement service and application upgrades and changes.', 
CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP());